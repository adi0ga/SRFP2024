...
def model_build(config_a):
    num_layers,size_layers,activation,lr=config_a
    ...
    model.compile("adam", lr=lr,metrics=["accuracy"],loss_weights=[0.01,1,1])
    return model
...
dim_activation=Categorical(["tanh","relu","sin","sigmoid"],name="activation")
dim_lr=Real(0.001,0.005,name="lr")
num_iter=12
dimensions=[dim_num_layers,dim_size_layers,dim_activation,dim_lr]
default_0=[3,50,"relu",0.001]
@use_named_args(dimensions=dimensions)
def fitness(num_layers,size_layers,activation,lr):
    global ITERATION
    config_a=[int(num_layers),int(size_layers),activation,lr]
    print("run No:",ITERATION)
    print(config_a)
